func (m *default{{.upperStartCamelObject}}Model) FindOneBy{{.upperField}}(ctx context.Context, {{.in}}) (*{{.upperStartCamelObject}}, error) {
	...
	err := m.QueryRowIndexCtx(ctx, &resp, {{.cacheKeyVariable}}, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where {{.originalField}} where delete_ts=0 limit 1", {{.lowerStartCamelObject}}Rows, m.table)
	...
	query := fmt.Sprintf("select %s from %s where {{.originalField}} AND delete_ts=0 limit 1", {{.lowerStartCamelObject}}Rows, m.table )
